  ...
  <v-container v-if='assignment'>
    ...
    >
      ...
      <div class='text-center'>
        Body: {{ assignment.task_variants[tab].max_points }}
      </div>
      ...
      <v-carousel-item v-for='task in assignment.task_variants' :key='task.id'>
        <div>
          ...
          <div class='d-flex flex-column align-center'>
            <h3>Riešenie:</h3>
            <SolutionEditor class='mb-10' :value='task.user_solution' :disabled='assignment.finished_at !== null' :correct='task.correct' @update='update'/>
          </div>
...
const update = async (value) => {
  const response = await ky.post(`assignment/${props.id}/${tab.value+1}`, {
    json: {
      solution: value,
    },
  }).json()
